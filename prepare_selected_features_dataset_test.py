...
for tumor_name in tumor_names:
	...
	test_chrom_filename = tumor_name + '/' + tumor_name + '_chrom.txt'
	test_truth_filename = tumor_name + '/' + tumor_name + '_truth.txt'
	test_data_filename = tumor_name + '/' + tumor_name + '_data.txt'
	test_data_mask_filename = tumor_name + '/' + tumor_name + '_data_mask.txt'


	np.savetxt(test_chrom_filename, clean_dataset[:,0:2], fmt='%d', delimiter='\t', header='chrom\tloc', comments='#')
	np.savetxt(test_truth_filename, clean_dataset[:,2], fmt='%d', delimiter='\t', header='truth', comments='#')
	np.savetxt(test_data_filename, clean_dataset[:,3:], fmt='%.3f', delimiter='\t', header=selected_features, comments='#')
	np.savetxt(test_data_mask_filename, clean_mask[:,:], fmt='%d', delimiter='\t', header=selected_features, comments='#')
